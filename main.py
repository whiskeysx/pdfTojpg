...
def convert_pdf_to_jpg(pdf_files, output_folders, poppler_path, progress_callback, output_base_folder, compress_images_enabled):
    try:
        ...
        for idx, (pdf_file, (output_folder, base_name)) in enumerate(zip(pdf_files, output_folders)):
            ...
            # Compress images if the option is enabled
            if compress_images_enabled.get():
        ...
        progress_message = f"Conversion {'and compression ' if compress_images_enabled.get() else ''}complete in {elapsed_time:.2f} seconds"
        messagebox.showinfo("Success", "All PDFs converted successfully!")
...
def start_conversion():
    ...
    threading.Thread(target=convert_pdf_to_jpg, args=(pdf_files, output_folders, poppler_path, update_progress, output_base_folder, compress_images_enabled)).start()
...
# Main GUI Window
def create_gui():
    global root, pdf_path_entry, output_path_entry, convert_button, progress_bar, status_label, compression_format, chapter_structure_var, compress_images_enabled
    ...
    # Checkbox for enabling/disabling compression
    compress_images_enabled = BooleanVar()
    compress_checkbox = Checkbutton(root, text="Enable Compression", variable=compress_images_enabled, font=("Helvetica", 12))
...
# Define Poppler path
poppler_path = r'C:\poppler-24.07.0\Library\bin'  # Adjust this to your actual Poppler "bin" path
